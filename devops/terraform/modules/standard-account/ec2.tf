...
resource "aws_security_group" "ec2_security" {
  name        = "calori-${var.account_name}-ec2-security-group"
...
resource "aws_iam_role" "ec2_iam_role" {
  name               = "calori-${var.account_name}-instance-role"
  assume_role_policy = data.aws_iam_policy_document.ec2_assume_role.json
    managed_policy_arns = [
      aws_iam_policy.s3_distribution_bucket_policy.arn,
      aws_iam_policy.calori_secrets_manager_policy.arn,
      aws_iam_policy.ec2_cloudwatch_policy.arn
  ]
}

resource "aws_iam_instance_profile" "calori_node" {
  name = "calori-${var.account_name}-ec2-profile"
...
resource "aws_instance" "ec2_calori_instance" {
  ami                         = data.aws_ami.ubuntu.id
  instance_type               = "t2.micro"
  key_name                    = "${var.aws_key_name}"
  vpc_security_group_ids      = [aws_security_group.ec2_security.id]
  subnet_id                   = aws_subnet.public_subnet.id
  iam_instance_profile        = aws_iam_instance_profile.calori_node.name
  associate_public_ip_address = true
  user_data                   = data.cloudinit_config.server_config.rendered
  user_data_replace_on_change = true

  tags = {
    Name = "calori-${var.account_name}-instance"
  }
  lifecycle {
